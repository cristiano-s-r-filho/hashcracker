...
const CS_SIZE: array<u32, 4> = array<u32, 4>(26u, 26u, 10u, 62u);
...
fn hmac_sha256(password: array<u32, 4>, pwd_len: u32, msg: ptr<function, array<u32, 16>>, msg_len: u32) -> array<u32, 8> {
...
fn pbkdf2_sha256(password: array<u32, 4>, pwd_len: u32, iterations: u32) -> array<u32, 8> {
...
fn char_from_digit(d: u32, cs: u32) -> u32 {
    if (cs == 0u) { return d + 97u; }
    else if (cs == 1u) { return d + 65u; }
    else if (cs == 2u) { return d + 48u; }
    else {
...
fn index_to_mask_str(index: u32) -> array<u32, 4> {
    var pwd: array<u32, 4>;
    var remaining = index;
    for (var i = 0u; i < 4u; i++) {
        if (i < config.mask_len) {
            let cs_id = config.mask[i];
            let sz = CS_SIZE[cs_id];
            let d = remaining % sz;
            pwd[i] = char_from_digit(d, cs_id);
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if (atomicLoad(&config.found_flag) != 0u) { atomicAdd(&progress.count, 1u); return; }    let pwd = index_to_mask_str(index);
    let iterations = config.target_hash_extra[0];
    let hash = pbkdf2_sha256(pwd, config.mask_len, iterations);
