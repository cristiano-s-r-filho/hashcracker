...
fn apr1_hash(pwd: array<u32, 4>, pwd_len: u32) -> array<u32, 4> {
    ...
    // Build digest_b: password + salt + password
    for (var i = 0u; i < 64u; i++) { msg[i] = 0u; }
    pos = 0u;
    ...
    // Now build full initial digest_a: digest_a_init + digest_b interleaved + bit processing
    // Start fresh message
    for (var i = 0u; i < 64u; i++) { msg[i] = 0u; }
    pos = 0u;

    // digest_a_init = password + "$1$" + salt
    for (var i = 0u; i < pwd_len; i++) {
        set_byte(&msg, pos + i, pwd[pwd_len - 1u - i] & 0xFFu);
    }
    pos += pwd_len;
    set_byte(&msg, pos, 0x24u); pos++;
    set_byte(&msg, pos, 0x61u); pos++;
    set_byte(&msg, pos, 0x70u); pos++;
    set_byte(&msg, pos, 0x72u); pos++;
    ...
    for (var i = 0u; i < 1000u; i++) {
        ...
        // Add salt if round not divisible by 3
        if ((i % 3u) != 0u) {
        ...
        // Add password if round not divisible by 7
        if ((i % 7u) != 0u) {
            for (var k = 0u; k < pwd_len; k++) {
                set_byte(&msg, pos + k, pwd[pwd_len - 1u - k] & 0xFFu);
            }
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let hash = apr1_hash(pwd, config.password_len);
