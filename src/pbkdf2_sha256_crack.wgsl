...
fn pack_be(data: array<u32, 16>, data_bytes: u32) -> array<u32, 16> {
    var block: array<u32, 16>;
    for (var i = 0u; i < 16u; i++) { block[i] = 0u; }
    for (var i = 0u; i < data_bytes && i < 64u; i++) {
        let src_word = i / 4u;
        let src_shift = (3u - (i % 4u)) * 8u;
        let byte = (data[src_word] >> src_shift) & 0xFFu;
        let dst_word = i / 4u;
        let dst_shift = (3u - (i % 4u)) * 8u;
        block[dst_word] |= byte << dst_shift;
    }
    return block;
}
...
fn pbkdf2_sha256(password: array<u32, 4>, pwd_len: u32, iterations: u32) -> array<u32, 8> {
    // Build U1 = HMAC(password, salt || 0x00000001)
    var msg: array<u32, 16>;
    for (var i = 0u; i < 16u; i++) { msg[i] = 0u; }

    // Pack salt bytes into msg (big-endian)
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let hash = pbkdf2_sha256(pwd, config.password_len, iterations);
