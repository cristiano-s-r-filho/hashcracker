...
fn sha512_init_state() -> array<u64, 8> {
...
fn sha512_block_raw(state: ptr<function, array<u64, 8>>, block: ptr<function, array<u64, 16>>) {
    sha512_block(state, *block);
}

fn hmac_sha512(pwd: array<u32, 4>, len: u32) -> array<u64, 8> {
    var ipad_block: array<u64, 16>;
    var opad_block: array<u64, 16>;
    for (var i = 0u; i < 16u; i++) {
        ipad_block[i] = u64(0x3636363636363636u);
        opad_block[i] = u64(0x5C5C5C5C5C5C5C5Cu);
    }

    for (var i = 0u; i < len; i++) {
        let ch = pwd[len - 1u - i] & 0xFFu;
        let byte_pos = i;
        let word_idx = byte_pos / 8u;
        let shift = (7u - (byte_pos % 8u)) * 8u;
        let mask = u64(0xFFu) << shift;
        ipad_block[word_idx] = (ipad_block[word_idx] & ~mask) | (u64(0x36u ^ ch) << shift);
        opad_block[word_idx] = (opad_block[word_idx] & ~mask) | (u64(0x5Cu ^ ch) << shift);
    }

    var state = sha512_init_state();
    sha512_block(&state, ipad_block);

    var block1: array<u64, 16>;
    for (var i = 0u; i < 16u; i++) { block1[i] = u64(0u); }
    ...
    for (var i = 0u; i < config.salt_len; i++) {
        ...
        let word_idx = byte_pos / 8u;
        let shift = (7u - (byte_pos % 8u)) * 8u;
        block1[word_idx] |= u64(ch) << shift;
    }

    let pad_byte = config.salt_len;
    let pad_word = pad_byte / 8u;
    let pad_shift = (7u - (pad_byte % 8u)) * 8u;
    block1[pad_word] |= u64(0x80u) << pad_shift;

    block1[15u] = u64(128u + config.salt_len) * u64(8u);

    sha512_block(&state, block1);

    var inner_hash: array<u64, 8>;
    for (var i = 0u; i < 8u; i++) {
        inner_hash[i] = state[i];
    }

    state = sha512_init_state();
    sha512_block(&state, opad_block);

    var block2: array<u64, 16>;
    for (var i = 0u; i < 16u; i++) { block2[i] = u64(0u); }

    for (var i = 0u; i < 8u; i++) {
        block2[i] = inner_hash[i];
    }

    let outer_pad_byte = 64u;
    let outer_pad_word = outer_pad_byte / 8u;
    let outer_pad_shift = (7u - (outer_pad_byte % 8u)) * 8u;
    block2[outer_pad_word] |= u64(0x80u) << outer_pad_shift;

    block2[15u] = u64(128u + 64u) * u64(8u);

    sha512_block(&state, block2);
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let pwd = index_to_password(index, config.password_len);
    let hash = hmac_sha512(pwd, config.password_len);
    if (check_match(hash)) {
