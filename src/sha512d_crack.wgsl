...
fn sha512(pwd: array<u32, 4>, len: u32) -> array<u64, 8> {
    ...
    block[15u] = u64(len + config.salt_len) * u64(8u);

    sha512_block(&state, block);
    return state;
}

fn sha512d_inner(input: array<u64, 8>) -> array<u64, 8> {
    var state: array<u64, 8>;
    ...
    for (var i = 0u; i < 8u; i++) { block[i] = input[i]; }

    block[8u] = u64(0x80u) << 56u;
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let h1 = sha512(pwd, config.password_len);
    let hash = sha512d_inner(h1);
    if (check_match(hash)) {
