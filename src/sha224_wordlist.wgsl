...
// Read word from word_buf for wordlist mode
fn word_entry_to_pwd(entry: WordEntry) -> array<u32, 4> {
    var pwd: array<u32, 4>;
    pwd[0u] = entry.chars[0u]; pwd[1u] = entry.chars[1u];
    pwd[2u] = entry.chars[2u]; pwd[3u] = entry.chars[3u];
    return pwd;
}
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let pwd = word_entry_to_pwd(entry);
    let hash = sha224(pwd, entry.len);
