...
fn sha256_bytes(data: array<u32, 64>, len: u32) -> array<u32, 8> {
    ...
    // SHA-256 length: high 32 bits in block[14], low 32 bits in block[15]
    block[14u] = 0u;
    block[15u] = len * 8u;
    sha256_block(&state, block);
...
fn hmac_sha256(pwd: array<u32, 4>, len: u32) -> array<u32, 8> {
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let hash = hmac_sha256(pwd, config.password_len);
