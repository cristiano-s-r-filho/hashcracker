// NTLM = MD4(UTF-16LE(password)) — Wordlist variant
...
fn ntlm_hash(pwd: array<u32, 5>, len: u32) -> array<u32, 4> {
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let hash = ntlm_hash(entry.chars, entry.len);
    ...
    if (found) {
        ...
        for (var i = 0u; i < entry.len; i = i + 1u) {
            config.found_password[i] = entry.chars[i];
        }
    }
