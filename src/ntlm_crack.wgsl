...
// MD4: 3 rounds of 16 steps (48 total), little-endian, 4-word state
...
// MD4 F, G, H functions (same as MD5's F, G but H is different)
...
// MD4 round constants (Round 1: 0, Round 2: 0x5A827999, Round 3: 0x6ED9EBA1)
const MD4_K: array<u32, 3> = array<u32, 3>(0u, 0x5A827999u, 0x6ED9EBA1u);

// MD4 round shift amounts
const MD4_S: array<u32, 48> = array<u32, 48>(
    // Round 1
    3u, 7u, 11u, 19u, 3u, 7u, 11u, 19u,
    3u, 7u, 11u, 19u, 3u, 7u, 11u, 19u,
    // Round 2
    3u, 5u, 9u, 13u, 3u, 5u, 9u, 13u,
    3u, 5u, 9u, 13u, 3u, 5u, 9u, 13u,
    // Round 3
    3u, 9u, 11u, 15u, 3u, 9u, 11u, 15u,
    3u, 9u, 11u, 15u, 3u, 9u, 11u, 15u,
);

// MD4 round indices (which message word each step uses)
...
fn ntlm_hash(pwd: array<u32, 4>, len: u32) -> array<u32, 4> {
    // Convert password to UTF-16LE bytes, then pad for MD4
    var msg: array<u32, 16>;
    for (var i = 0u; i < 16u; i = i + 1u) { msg[i] = 0u; }

    // Encode password as UTF-16LE, 2 bytes per character
    var bit_len: u32 = 0u;
    for (var i = 0u; i < len; i = i + 1u) {
        let ch = pwd[len - 1u - i];
        // Each character as u16 stored little-endian in msg
        let word_idx = (i * 2u) / 4u;
        let byte_in_word = (i * 2u) % 4u;
        // Low byte of UTF-16LE character
        msg[word_idx] = msg[word_idx] | (ch << (byte_in_word * 8u));
        // High byte is 0 for ASCII characters
        // If char > 127 we'd need actual UTF-16, but for ASCII this is fine
        bit_len = bit_len + 16u;
    }

    // MD4 padding: append 0x80 byte
    ...
    // Append bit length in last 2 words (little-endian)
    msg[14u] = bit_len;
    msg[15u] = 0u;
    ...
    // NTLM result is MD4 state (little-endian u32s)
    return state;
}
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if (index >= config.range_end) {
        return;
    }
    if (atomicLoad(&config.found_flag) != 0u) { atomicAdd(&progress.count, 1u); return; }

    let pwd = index_to_password(index, config.password_len);
    let hash = ntlm_hash(pwd, config.password_len);

    var found = false;
    for (var t = 0u; t < config.num_targets; t++) {
        if (hash[0u] == targets[t].hash[0u] &&
            hash[1u] == targets[t].hash[1u] &&
            hash[2u] == targets[t].hash[2u] &&
            hash[3u] == targets[t].hash[3u]) {
            found = true;
        }
    }
    ...
    if (found) {
        ...
        for (var i = 0u; i < config.password_len; i = i + 1u) {
