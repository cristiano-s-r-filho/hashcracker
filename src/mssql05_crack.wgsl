...
fn sha256(pwd: array<u32, 4>, len: u32) -> array<u32, 8> {
    ...
    // Uppercase password after salt
    for (var i = 0u; i < len && i < 16u; i++) {
        let c = pwd[len - 1u - i] & 0xFFu;
        let upper_c = select(c, c - 32u, c >= 97u && c <= 122u);
        let dst_byte = config.salt_len + i;
        let dst_word = dst_byte / 4u;
        let dst_shift = (3u - (dst_byte % 4u)) * 8u;
        block[dst_word] |= upper_c << dst_shift;
    }

    // Padding: 0x80 after salt + password
    let pad_byte = config.salt_len + len;
    let pad_word = pad_byte / 4u;
    let pad_shift = (3u - (pad_byte % 4u)) * 8u;
    block[pad_word] |= 0x80u << pad_shift;

    // Length in bits (big-endian)
    block[15u] = (config.salt_len + len) * 8u;
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let hash = sha256(pwd, config.password_len);
